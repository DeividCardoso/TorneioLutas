...
namespace TorneioLutas.Service.Models
{
    public class Luta
    {
        ...
        {
            if (lutador1.ArtesMarciais.Count > lutador2.ArtesMarciais.Count) return Lutador1;
            else if (lutador2.ArtesMarciais.Count > lutador1.ArtesMarciais.Count) return Lutador2;
            else return DesempatarNumeroLutas(Lutador1, Lutador2);
        }

        private Lutador DesempatarNumeroLutas(Lutador lutador1, Lutador lutador2)
        {
            if (lutador2.TotalLutas > lutador1.TotalLutas) return Lutador2;
